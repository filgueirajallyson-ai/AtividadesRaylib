...
float distanciaEntre(Entidade *a, Entidade *b){
...
void ordenarPorDistancia(Entidade *jogador) {
    for (int i = 1; i < totalEntidades - 1; i++) {
      for (int j = i + 1; j < totalEntidades; j++) {
        float distanciaI = distanciaEntre(jogador, vetorEntidades[i]);
        float distanciaJ = distanciaEntre(jogador, vetorEntidades[j]);
          if (distanciaJ < distanciaI){
            Entidade *temp = vetorEntidades[i];
             vetorEntidades[i] = vetorEntidades[j];
              vetorEntidades[j] = temp;
          }
      }
    }
}
